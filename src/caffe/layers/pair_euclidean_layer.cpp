...
void PairEuclideanLayer<Dtype>::Reshape(
  ...
  CHECK(bottom.size()<=3) << "number of bottom blobs can only be 1,2 or 3 ";  
 if (bottom.size() == 2) this->output_labels_ = true;
  ...
  vector<int> datashape(bottom[0]->shape());
  datashape[0] /= 2;
  CHECK_EQ(bottom[0]->shape(0)%2, 0);
  ...
  if (this->output_labels_) {
    ...
    vector<int> shape(1, top[0]->shape(0));
