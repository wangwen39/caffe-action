...
void ReidPrefetchingDataLayer<Dtype>::LayerSetUp(
  ...
  // cudaMalloc calls when the main thread is running. In some GPUs this
  // seems to cause failures if we do not so.
  CHECK(top.size() <= 3) << "number of top blobs can only be 1, 2 or 3";
  if (top.size() >= 2) {
     this->output_labels_ = true; 
     if (top.size() == 3) {
       this->output_overlaps_ = true;
     } else {
       this->output_overlaps_ = false;
     }
  } else {
  ...
  for (int i = 0; i < prefetch_.size(); ++i) {
    ...
    if (this->output_overlaps_) {
      prefetch_[i]->overlap_.mutable_cpu_data();
      prefetch_[i]->overlapp_.mutable_cpu_data();
    }
  }
#ifndef CPU_ONLY
  if (Caffe::mode() == Caffe::GPU) {
    for (int i = 0; i < prefetch_.size(); ++i) {
      ...
      if (this->output_overlaps_) {
        prefetch_[i]->overlap_.mutable_gpu_data();
        prefetch_[i]->overlapp_.mutable_gpu_data();
      }
    }
  }
#endif
  ...
  try {
    while (!must_stop()) {
      ...
      if (Caffe::mode() == Caffe::GPU) {
        ...
        if (this->output_overlaps_) {
          batch->overlap_.data().get()->async_gpu_push(stream);
          batch->overlapp_.data().get()->async_gpu_push(stream);
        }
        CUDA_CHECK(cudaStreamSynchronize(stream));
      }
#endif
...
void ReidPrefetchingDataLayer<Dtype>::Forward_cpu(
  ...
  if (this->output_overlaps_) {
    ...
    top[2]->Reshape(shape);
    // Copy the labels.
    caffe_copy(prefetch_current_->overlap_.count(),  prefetch_current_->overlap_.cpu_data(),  top[2]->mutable_cpu_data());
    caffe_copy(prefetch_current_->overlapp_.count(), prefetch_current_->overlapp_.cpu_data(), top[2]->mutable_cpu_data()+prefetch_current_->overlapp_.count());
  }
